...
def process_text(exp, emoji_dict, teen_dict, english_dict, wrong_lst):
  ...
  documents = new_sentence
  documents = regex.sub(r'\s+', ' ', new_sentence).strip()
  return documents
...
# cleaned final
def clean_text(text):
    document = func.process_text(text, emoji_dict, english_dict, teen_dict, wrong_lst)
    document = func.covert_unicode(document)
    # document = func.process_postag_thesea(document)
    document = func.remove_stopword(document, stopwords_lst)
    return document

# predict any restaurant
def predict_sentiment(data, restaurant_name):
  # get restaurnant information:
  df_restaurant = data.loc[data['restaurant'] == restaurant_name]
  ...
  df_restaurant['predict_class'] = y_pred_class

  result = df_restaurant
